...
{
    (void) result_capacity; // unused because we always realloc
...
static bool cmdline_parse_generic(char *start, size_t length, size_t cmdline_max, size_t *out_count, const char ***argv_ptr)
{
    ...
    for (char *c = start; c != &buf_start[length + 1]; c++)
    {
        ...
        if (*c == '\0')
        {
            if (strlen(start) > 0)
            {
                *argv_ptr = cmdline_dynamic_array_insert(*argv_ptr, cmdline_max, start, out_count);
                if (!*argv_ptr)
                    return false;
            }
...
{
    if (!cmdline_parse_generic(inbuf, length, 0, out_count, &inargv))
        return NULL;
    return inargv;
}
