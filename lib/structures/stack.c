...
void stack_init(downwards_stack_t *stack, void *mem_region_bottom, size_t size)
{
    ...
    uintptr_t stack_base = (uintptr_t) mem_region_bottom + size;

    *((uintptr_t *) &stack->base) = stack_base;
...
void *stack_grow(downwards_stack_t *stack, size_t size)
{
    ...
    if (unlikely(stack->head - (stack->base - stack->capacity) < size))
    {
...
{
    // high memory | base -----> head -----> base - capacity | low memory
    if (unlikely(stack->head - stack->base < size))
    {
