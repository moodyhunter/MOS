    ...
    list_foreach(vmap_t, vmap, parent->mm->mmaps)
    {
        switch (vmap->content)
        {
            case VMTYPE_KSTACK:
            {
                ...
                const vmblock_t block = mm_alloc_pages(child_p->mm->pagetable, vmap->blk.npages, MOS_ADDR_USER_STACK, VALLOC_DEFAULT, vmap->blk.flags);
            ...
            case VMTYPE_MMAP:
            {
                ...
                if (vmap->flags.fork_mode == VMAP_FORK_SHARED)
                {
                    ...
                    block.address_space = child_p->mm->pagetable;
                    mm_copy_maps(parent->mm->pagetable, block.vaddr, child_p->mm->pagetable, block.vaddr, block.npages, MM_COPY_DEFAULT);
            ...
            {
                vmap->flags.cow = true;
                const vmblock_t block = mm_make_cow_block(child_p->mm->pagetable, vmap->blk);
