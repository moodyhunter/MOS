...
ptr_t mmap_anonymous(ptr_t hint_addr, mmap_flags_t flags, vm_flags vm_flags, size_t n_pages)
{
    ...
    const vmblock_t block = mm_alloc_zeroed_pages(current_process->mm->pagetable, n_pages, hint_addr, valloc_flags, vm_flags);
...
bool munmap(ptr_t addr, size_t size)
{
    ...
    mm_unmap_pages(current_process->mm->pagetable, range_start, n_pages);
