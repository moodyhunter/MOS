...
{
    vmblock_t block = mm_alloc_pages(platform_info->kernel_mm.pagetable, n, MOS_ADDR_KERNEL_HEAP, VALLOC_DEFAULT, VM_RW);
    return block.vaddr;
}

static void slab_impl_free_page(ptr_t page, size_t n)
{
    mm_unmap_pages(platform_info->kernel_mm.pagetable, page, n);
}
