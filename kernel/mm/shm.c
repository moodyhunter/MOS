...
vmblock_t shm_allocate(size_t npages, vmap_fork_mode_t mode, vm_flags vmflags)
{
    ...
    mos_debug(shm, "allocating %zu SHM pages in address space " PTR_FMT, npages, owner->mm);
...
{
    process_t *owner = current_process;
    mos_debug(shm, "sharing %zu pages from address space " PTR_FMT " to address space " PTR_FMT, source.npages, source.address_space, owner->mm);
