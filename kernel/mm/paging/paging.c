...
vmblock_t mm_alloc_pages_at(paging_handle_t table, uintptr_t vaddr, size_t npages, vm_flags flags)
{
    ...
    vmblock_t block = { .address_space = table, .vaddr = vaddr, .npages = npages, .flags = flags };

    table = PGD_FOR_VADDR(vaddr, table);

    if (unlikely(table.pgd == 0))
    {
        mos_warn("cannot allocate %zd pages at " PTR_FMT ", pagetable is null", npages, vaddr);
        return (vmblock_t){ 0 };
    }

    spinlock_acquire(table.pgd_lock);
    pagemap_mark_used(table.um_page_map, vaddr, npages);
    const bool result = pmm_allocate_frames(npages, vmm_handle_pmalloc, &block);
    spinlock_release(table.pgd_lock);

    if (unlikely(!result))
    {
        mos_warn("could not allocate %zd physical pages", npages);
        return (vmblock_t){ 0 };
    }
