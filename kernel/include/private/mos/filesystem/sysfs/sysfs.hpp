...
#define SYSFS_DYN_ITEMS(_name, _iterate_fn, _lookup_fn) { .type = SYSFS_DYN, .iterate = _iterate_fn, .lookup = _lookup_fn }
#define SYSFS_DYN_DIR(_name, _iterate_fn, _lookup_fn, _create_fn) { .type = SYSFS_DYN, .iterate = _iterate_fn, .lookup = _lookup_fn, .create = _create_fn }
// clang-format on
