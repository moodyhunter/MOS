...
__startup_code should_inline void mos_startup_memzero(void *start, size_t size)
{
    u8 *ptr = start;
...
__startup_code should_inline size_t mos_startup_strlen(const char *str)
{
    size_t len = 0;
    while (str[len])
        len++;
...
__startup_code should_inline void mos_startup_map_pages(ptr_t vaddr, ptr_t paddr, size_t npages, vm_flags flags)
{
    for (size_t i = 0; i < npages; i++)
        mos_startup_map_single_page(vaddr + i * MOS_PAGE_SIZE, paddr + i * MOS_PAGE_SIZE, flags);
}

__startup_code should_inline void mos_startup_map_bytes(ptr_t vaddr, ptr_t paddr, size_t nbytes, vm_flags flags)
{
...
__startup_code should_inline void mos_startup_map_identity(ptr_t paddr, size_t nbytes, vm_flags flags)
{
    paddr = ALIGN_DOWN_TO_PAGE(paddr);
    mos_startup_map_bytes(paddr, paddr, nbytes, flags);
}

__startup_code should_inline void mos_startup_map_bios(ptr_t paddr, size_t nbytes, vm_flags flags)
{
    paddr = ALIGN_DOWN_TO_PAGE(paddr);
