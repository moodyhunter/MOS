...
{
    MOS_UNUSED(data);
    pr_info2("%*s%s: %zu%s", depth * 4, "", dentry_name(dentry), dentry->refcount.load(), mountroot ? " (mountroot)" : "");
}
...
MOS_TEST_DECL_PTEST(vfs_mount_test, "Mount %s in %s, with rootfs: %d", const char *fs, const char *mountpoint, bool rootfs)
{
    ...
    if (rootfs)
    {
        ok = vfs_mount("none", "/", "tmpfs", "");
        MOS_TEST_ASSERT(ok == 0, "failed to mount tmpfs on /");
    }

    ok = vfs_mkdir(mountpoint);
    ...
    ok = vfs_mount("none", mountpoint, fs, "");
