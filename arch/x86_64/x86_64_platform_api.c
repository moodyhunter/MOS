...
{
    return (mm_context_t){ .pgd = 0, .pgd_lock = 0, .um_page_map = 0 };
}
...
void platform_mm_map_pages(mm_context_t table, ptr_t vaddr, ptr_t paddr, size_t n_pages, vm_flags flags)
{
    MOS_UNUSED(table);
    MOS_UNUSED(vaddr);
    MOS_UNUSED(paddr);
...
void platform_mm_unmap_pages(mm_context_t table, ptr_t vaddr, size_t n_pages)
{
    MOS_UNUSED(table);
    MOS_UNUSED(vaddr);
    MOS_UNUSED(n_pages);
...
ptr_t platform_mm_get_phys_addr(mm_context_t table, ptr_t vaddr)
{
    MOS_UNUSED(table);
    MOS_UNUSED(vaddr);
    return 0;
...
void platform_mm_flag_pages(mm_context_t table, ptr_t vaddr, size_t n, vm_flags flags)
{
    MOS_UNUSED(table);
    MOS_UNUSED(vaddr);
    MOS_UNUSED(n);
...
vm_flags platform_mm_get_flags(mm_context_t table, ptr_t vaddr)
{
    MOS_UNUSED(table);
    MOS_UNUSED(vaddr);
    return 0;
