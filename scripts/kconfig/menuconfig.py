...
_MAIN_HELP_LINES = """
[Space/Enter] Toggle/enter  [ESC] Leave menu / Quit    [S] Save
[O] Load                    [h] Symbol info            [/] Jump to symbol
...
def _menuconfig(stdscr):
    ...
    while True:
        ...
        if c == curses.KEY_RESIZE:
            ...
            if not _change_node(sel_node, may_leave=False):
            ...
            if not _enter_menu(sel_node):
                _change_node(sel_node, may_leave=c == "\n")
        ...
        elif c in (curses.KEY_LEFT, curses.KEY_BACKSPACE, _ERASE_CHAR, "\x1B"):  # \x1B = ESC
...
def _change_node(node, may_leave=True):
    ...
    if _is_y_mode_choice_sym(sc) and not node.list and may_leave:
