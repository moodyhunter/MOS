...
struct printf_flags_t
{
    ...
    bool space_if_positive = false; // prepend space if positive
    bool pad_with_zero = false;     // padding with zero
...
static int printf_diouxX(char *buf, u64 number, printf_flags_t *pflags, char conv, size_t *size_left)
{
    ...
    s32 precision_padding = std::max(pflags->precision - n_digits, 0);
    s32 width_to_pad = std::max(pflags->minimum_width - strlen(num_prefix_buf) - precision_padding - n_digits, 0lu);
...
static int printf_cs(char *buf, const char *data, printf_flags_t *pflags, char conv, size_t *psize_left)
{
    ...
    if (pflags->has_explicit_precision)
        printed_len = std::min(printed_len, pflags->precision);

    s32 width_to_pad = std::max(pflags->minimum_width - printed_len, 0);
        ...
        switch (*format)
        {
            ...
            case 'X':
            {
                ...
                switch (flags.length)
                {
                    ...
                    case LM__j: value = va_arg(args.real, intmax_t); break;
                    default: MOS_LIB_UNREACHABLE();
                }
